...
readonly LOG_FILE="${LOG_FILE:-${SCRIPT_DIR}/etcd-load-tools.log}"
...
get_master_nodes() {
    local nodes
    nodes=$(oc get nodes -o name --no-headers 2>/dev/null | grep -E '/(.*-)?master(-.*)?$' | sed 's|node/||') || return 1
    [[ -n "$nodes" ]] || { log ERROR "No master nodes found"; return 1; }
    echo "$nodes"
}
...
run_container_test() {
    local test_name="$1"
    local -n config="$2"
    local node_mode="$3"  # "single" or "all"
    
    local image="${config[0]}"
    local command="${config[1]}"
    local mount_path="${config[2]}"
    local extra_flags="${config[3]}"
